...
class PrintProductLabel(models.TransientModel):
    ...
    eye = fields.Boolean(string='Eye', default=True)
    bridge = fields.Boolean(string='Bridge', default=True)
    temple = fields.Boolean(string='Temple', default=True)
    color = fields.Boolean(string='Color', default=True)
    upc = fields.Boolean(string='UPC')
    sku = fields.Boolean(string='SKU')
    ...
    def action_print(self):
        ...
        action = self.env.ref(self.template).with_context(discard_logo_check=True).report_action(labels)
