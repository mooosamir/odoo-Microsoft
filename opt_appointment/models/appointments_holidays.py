...
class AppointmentsHolidays(models.Model):
    _name = 'appointments.holidays'
    _description = "Appointment Holidays"
    ...
    holiday_open = fields.Char(string='Modified Hours')
    holiday_close = fields.Char(string='Modified Hours')
    opening_time = fields.Datetime('Modified Opening Time')
    closing_time = fields.Datetime('Modified Closing Time')
    holiday_closed = fields.Boolean('Closed')
    appointments_hours_id = fields.Many2one('appointments.hours', string="Appointment Hour")

    is_original = fields.Boolean('Original')
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company, required=True)

    @api.onchange('holiday_closed')
    def _onchange_holiday_closed(self):
        self.holiday_close = self.holiday_open = ""
    ...
    def write(self, values):
        if not values.get('holiday_closed'):
            values['holiday_closed'] = self.holiday_closed
        if values.get('date'):
            if not values['holiday_closed']:
        ...
        if values.get('company_id'):
            ...
            if not values.get('holiday_open') and not values['holiday_closed']:
                values['holiday_open'] = self.holiday_open
            if not values.get('holiday_close') and not values['holiday_closed']:
                values['holiday_close'] = self.holiday_close
        else:
            company_event_tz = self.company_event_tz

        if values.get('holiday_closed'):
            open_datetime = datetime.strptime('12:00 am', '%I:%M %p')
            ...
            close_datetime = datetime.strptime("11:59 pm", '%I:%M %p')
        ...
        return super(AppointmentsHolidays, self).write(values)

    # copy Not required, as the create function is running.

    @api.model
    def create(self, values):
        if values.get('date') and type(values['date']) != date:
            values['date'] = datetime.strptime(values['date'], '%Y-%m-%d').date()
        if values.get('company_id'):
            company_event_tz = self.env['res.company'].search([('id', '=', values['company_id'])]).timezone
        ...
        if values['recurrency']:
            ...
            new_values = {
                ...
                'holiday_closed': values['holiday_closed'],
                'recurrency': False,
            ...
            if values['rrule_type'] == 'daily':
                if values['end_type'] == 'count':
                    for i in range(0, values['count'], values['interval']):
                        ...
                        super(AppointmentsHolidays, self).create(new_values)
                else:
            ...
            return super(AppointmentsHolidays, self).create(values)
        else:
            return super(AppointmentsHolidays, self).create(values)
