odoo.define('patient_engagement.online_request_systray', function (require) {
"use strict";
    ...
    var NotificationMenu = Widget.extend({
        name: 'online_request_notifications_menu',
        template:'systray.online_request.NotificationsMenu',
        events: {
            'click .online_request_notification': '_OpenActionWindow',
        },
        start: function () {
            this.counter = 0;
            var mailBus = this.call('mail_service', 'getMailBus');
            mailBus.on('online_request_notification_updated', this, this._updateCounter);
...
//            this.call('mail_service', 'getMailBus').trigger('twilio_notification_updated', {message_seen: 1});
        ...
        _OpenActionWindow: function () {
            this.do_action({
                type: 'ir.actions.act_window',
                view_mode: 'kanban,list,form',
                views: [[false, 'kanban'], [false, 'list'], [false, 'form']],
                target: 'current',
                res_model: 'online.requests',
                domain: [['state','!=', 'deleted'],['state','!=', 'removed']],
            });
        },
        _getCounter: function (data) {
            var self = this;
            return self._rpc({
                model: 'online.requests',
